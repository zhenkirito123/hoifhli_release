...
class Logger:
    ...
    def log(self, energy, E_fc, E_dis, E_pen, E_prior, E_spen, step, show=False):
        """
        ...
        self.writer.add_scalar("Energy/prior", E_prior.mean(), step)
        ...
        if show:
            print(
                f"Step %d energy: %f  fc: %f  dis: %f  pen: %f"
                % (step, energy.mean(), E_fc.mean(), E_dis.mean(), E_pen.mean())
            )
            print(
                f"success: %f  fc: %f  dis: %f  pen: %f"
                % (
                    success.float().mean(),
                    success_fc.float().mean(),
                    success_dis.float().mean(),
                    success_pen.float().mean(),
                )
            )
