...
class Annealing:
    ...
    def try_step(self):
        ...
        # switch_mask = torch.rand(1).item() < self.switch_possibility

        # if switch_mask:
        #     self.hand_model.other_idx = torch.randint(low=0, high=5, size=(1,)).item()

        # thumb_point_indices = torch.randint(len(self.hand_model.thumb_contact_indices), size=[batch_size, 2], device=self.device)
        # other_point_indices = torch.randint(len(self.hand_model.other_contact_indices[self.hand_model.other_idx]), size=[batch_size, n_contact - 2], device=self.device)
        # contact_point_indices = torch.cat([thumb_point_indices, other_point_indices], dim=1)
